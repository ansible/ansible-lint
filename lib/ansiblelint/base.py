...
class AnsibleLintRule(object):
    ...
    def matchlines(self, file, text):
        ...
        for (prev_line_no, line) in enumerate(text.split("\n")):
            ...
            rule_id_list = utils.get_rule_skips_from_line(line)
    ...
    def matchtasks(self, file, text):
        ...
        yaml = utils.append_skipped_rules(yaml, text, file['type'])
        ...
        for task in utils.get_normalized_tasks(yaml, file):
            ...
            matches.append(Match(task[utils.LINE_NUMBER_KEY], task_msg,
                           file['path'], self, message))
        return matches
...
class RulesCollection(object):
    ...
    def __init__(self, rulesdirs=None):
        ...
        for rulesdir in self.rulesdirs:
            self.extend(utils.load_plugins(rulesdir))
...
class Runner(object):
    ...
    def _update_exclude_paths(self, exclude_paths):
        if exclude_paths:
            ...
            paths = utils.expand_paths_vars(exclude_paths)
    ...
    def run(self):
        ...
        for playbook in self.playbooks:
            ...
            if playbook[1] == 'role':
                continue
            files.append({'path': utils.normpath(playbook[0]), 'type': playbook[1]})
        visited = set()
        while (visited != self.playbooks):
            for arg in self.playbooks - visited:
                for child in utils.find_children(arg, self.playbook_dir):
