...
class NodeAnnotator(NodeVisitor):
    ...
    def seek(self, token_type: str) -> Tuple[List[Token], Optional[Token]]:
        before_tokens, token = self.tokens.seek(token_type)
        for before_token in before_tokens:
            if before_token.jinja_token is None:
                # lexer.wrap() ignored whitespace, a comment, or a raw tag
                pass
            else:
                # This is surprising. We went past an actual token?
                pass
        return before_tokens, token
    ...
    @contextmanager
    def token_pair(self, node: nodes.Node, block: Optional[str] = None):
    ...
    def visit_Output(self, node: nodes.Template) -> None:
        ...
        for child_node in node.iter_child_nodes():
            ...
            with self.token_pair(child_node):
                self.visit(child_node)
    ...
    def visit_Block(self, node: nodes.Block) -> None:
        ...
        """
        with self.token_pair(node, block="block"):
            self.seek(node.name)
        ...
        for child_node in node.body:
            self.visit(child_node)
        with self.token_pair(node, block="endblock"):
            pass
    ...
    def visit_Extends(self, node: nodes.Extends) -> None:
        ...
        Example::
            ...
            {% extends name %}
        """
        with self.token_pair(node, block="extends"):
    ...
    def visit_Include(self, node: nodes.Include) -> None:
        ...
        """
        with self.token_pair(node, block="include"):
            self.visit(node.template)
    ...
    def visit_Import(self, node: nodes.Import) -> None:
        ...
        """
        with self.token_pair(node, block="import"):
            self.visit(node.template)
            self.seek("as")
    ...
    def visit_FromImport(self, node: nodes.FromImport) -> None:
        ...
        """
        with self.token_pair(node, block="from"):
            self.visit(node.template)
            self.seek("import")
    ...
    def visit_For(self, node: nodes.For) -> None:
        ...
        """
        with self.token_pair(node, block="for"):
        ...
        for child_node in node.body:
            self.visit(child_node)
        if node.else_:
            with self.token_pair(node, block="else"):
                pass
            for child_node in node.else_:
                self.visit(child_node)
        with self.token_pair(node, block="endfor"):
            pass

    def visit_If(self, node: nodes.If) -> None:
        """Visit an ``If`` block in the stream."""
        with self.token_pair(node, block="if"):
            self.visit(node.test)
        for child_node in node.body:
            self.visit(child_node)
        for elif_node in node.elif_:
            with self.token_pair(node, block="elif"):
        ...
        if node.else_:
            ...
            for child_node in node.else_:
                self.visit(child_node)
        with self.token_pair(node, block="endif"):
            pass

    def visit_With(self, node: nodes.With) -> None:
        """Visit a ``With`` statement (manual scopes) in the stream."""
        with self.token_pair(node, block="with"):
            first = True
        ...
        for child_node in node.body:
            self.visit(child_node)
        with self.token_pair(node, block="endwith"):
            pass
    ...
    def visit_ExprStmt(self, node: nodes.ExprStmt) -> None:
        ...
        """
        with self.token_pair(node, block="do"):
    ...
    def visit_Assign(self, node: nodes.Assign) -> None:
        ...
        Example::
            ...
            {% set var = value %}
        """
        with self.token_pair(node, block="set"):
            self.visit(node.target)
            self.seek("=")
            self.visit(node.node)
    ...
    def visit_AssignBlock(self, node: nodes.AssignBlock) -> None:
        ...
        for child_node in node.body:
            self.visit(child_node)
        with self.token_pair(node, block="endset"):
            pass
    ...
    def visit_FilterBlock(self, node: nodes.FilterBlock) -> None:
        ...
        """
        with self.token_pair(node, block="filter"):
            self.visit(node.filter)
        for child_node in node.body:
            self.visit(child_node)
        with self.token_pair(node, block="endfilter"):
            pass
    ...
    def visit_Macro(self, node: nodes.Macro) -> None:
        ...
        """
        with self.token_pair(node, block="macro"):
            self.seek(node.name)
            self.macro_signature(node)
        for child_node in node.body:
            self.visit(child_node)
        with self.token_pair(node, block="endmacro"):
            pass
    ...
    def visit_CallBlock(self, node: nodes.CallBlock) -> None:
        ...
        """
        with self.token_pair(node, block="call"):
            if node.args:
                self.macro_signature(node)
            self.visit(node.call)
        for child_node in node.body:
            self.visit(child_node)
        with self.token_pair(node, block="endcall"):
            pass
    ...
    def visit_Continue(
        ...
        with self.token_pair(node, block="continue"):
            pass
    ...
    def visit_Break(self, node: nodes.Break) -> None:  # pylint: disable=unused-argument
        ...
        with self.token_pair(node, block="break"):
            pass
    ...
    def visit_ScopedEvalContextModifier(
        ...
        if autoescape is None:
            ...
            self.generic_visit(node)
            return
        with self.token_pair(node, block="autoescape"):
            self.visit(autoescape)
        for child_node in node.body:
            self.visit(child_node)
        with self.token_pair(node, block="endautoescape"):
            pass
