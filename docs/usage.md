...
For optimal performance, Ansible-lint creates caches with installed roles and
collections under the runtime cache directory (typically `{project}/.ansible`
when not offline, or `~/.ansible` / `ANSIBLE_HOME` when `offline: true`).

Mocked modules and roles from `mock_modules` / `mock_roles` are written under
`{cache_dir}/ansible-lint-mocks/` so they never overwrite installed collection
content. That mock tree is added as a **fallback** on the collections/module
...
!!! warning "Recovering clobbered collections"

    Older ansible-lint releases wrote `mock_modules` stubs directly into
    `{cache_dir}/collections/`, which could replace real module files when
    `offline: true` used `~/.ansible`. If modules under
    `~/.ansible/collections/...` start with the lint mock marker, reinstall
