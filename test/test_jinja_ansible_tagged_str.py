...
from ansiblelint.rules.jinja import JinjaRule, ignored_re
...
class TestAnsibleTaggedStrErrorHandling:
    ...
    def test_jinja_cockpit_style_template_error_handling(self) -> None:
        ...
        tagged_error = AnsibleError('can only concatenate list (not "_AnsibleTaggedStr") to list')
        assert ignored_re.search(str(tagged_error)), f"_AnsibleTaggedStr error should be ignored: {tagged_error}"
        ...
        assert not ignored_re.search(str(normal_error)), f"Normal error should not be ignored: {normal_error}"

    @pytest.mark.skip(reason="Requires full environment setup")
    def test_jinja_template_generates_ansible_tagged_str_error_comprehensive(self) -> None:
    ...
    def test_jinja_ignore_patterns_comprehensive(self, error_message: str, should_be_ignored: bool) -> None:
